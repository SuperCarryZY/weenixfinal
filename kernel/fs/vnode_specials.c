...
static ssize_t chardev_file_read(vnode_t *file, size_t pos, void *buf,
                                 size_t count)
{
    // Unlock the file before calling the chardev's read to avoid deadlock
    vunlock(file);
    
    // Call the underlying chardev's read operation
    ssize_t result = file->vn_dev.chardev->cd_ops->read(file->vn_dev.chardev, pos, buf, count);
    
    // Lock the file again after the operation
    vlock(file);
...
static long chardev_file_write(vnode_t *file, size_t pos, const void *buf,
                               size_t count)
{
    // Unlock the file before calling the chardev's write to avoid deadlock
    vunlock(file);
    
    // Call the underlying chardev's write operation
    ssize_t result = file->vn_dev.chardev->cd_ops->write(file->vn_dev.chardev, pos, buf, count);
...
static long chardev_file_mmap(vnode_t *file, mobj_t **ret)
{
    // Defer to the underlying chardev's mmap operation
    return file->vn_dev.chardev->cd_ops->mmap(file, ret);
}

static long chardev_file_fill_pframe(vnode_t *file, pframe_t *pf)
{
    // Defer to the underlying chardev's fill_pframe operation
    return file->vn_dev.chardev->cd_ops->fill_pframe(file, pf);
}

static long chardev_file_flush_pframe(vnode_t *file, pframe_t *pf)
{
    // Defer to the underlying chardev's flush_pframe operation
